...
def get_saturation_temperature(P, Q, fluid):
    T_satK = PropsSI('T', 'P', P, 'Q', Q, fluid)
    return T_satK

def get_temperature(P, h, fluid):
    Temp = PropsSI('T', 'P', P, 'H', h, fluid)
    return Temp



### Main program
## 냉방부하 조건(Known Values) 입력(INPUT).
V_car = 60.0                    # 차량 속도 [km/h] 
t_amb = 27.0                    # 외기 공기온도[C]
t_air_evap_out = 6.13           # 증발기 출구 공기 건구온도[C]
rh_evap_in = 0.50               # 증발기 입구 공기 상대습도(%RH/100)
rh_evap_out = 0.99              # 증발기 출구 공기 상대습도(%RH/100)
AirVol = 450.0                  # 증발기 입구 풍량[CMH]
Pair_in = 101325                # 입구부 공기 압력 = 대기압[Pa]
Pair_out = 101325               # 출구부 공기 압력 = 대기압[Pa]

# Evaporator Cooling Load Calculation Condition (입력 결과에 따른 계산)
t_air_evap_in = t_amb + 0.0     # 증발기 입구 공기온도[C]. 건구온도. Heat Pickup 2.0[C]
t_air_cond_in = t_amb + 0.0     # 증발기 입구 공기온도[C]. 건구온도. Heat Pickup 0.0[C]

# 증발기 입/출구부 공기 습구온도(WB) 계산
twet_in = GetTWetBulbFromRelHum(t_air_evap_in, rh_evap_in, Pair_in)
twet_in = round(twet_in,1)  
twet_out = GetTWetBulbFromRelHum(t_air_evap_out, rh_evap_out, Pair_out)
twet_out = round(twet_out,1)
...
massflow_air = round(massflow_air, 1)
...
## 3. Refrigerant Flowrate Calculation
# 증발기 내부 유동 냉매 질량유동률 계산
# 3.1 Evaporator Inlet/Outlet Enthalpy Calculation
...
# PbarA = (Pkgcm2G * 0.980665) + 1.01325
fluid = 'R134a'                 # Input 냉매 종류
SC_cond = 5.0                  # 응축기 출구 과냉각도[℃]
SH_comp = 5.0                  # 압축기 입구 과열도[℃]
P1 = 2.0*0.980665+1.01325                        # 압축기 입구 압력[barA]
# T1 = 17.0                       # 압축기 입구 온도[C]
T1_sat = get_saturation_temperature(P1*1e5, 1, fluid) - 273
T1 = T1_sat + SH_comp          # 압축기 입구 온도[C] (과열도 적용)
h1 = get_enthalpy(T1+273, P1*1e5, fluid)

print(f'P1 = {P1:.2f} barA, T1_sat = {T1_sat:.1f} C , T1 = {T1:.1f} C, h1 = {h1:.1f} kJ/kg')

P2 = 15.7*0.980665+1.01325      # 압축기 출구 압력[barA]
T2 = 84.0                       # 압축기 출구 온도[K]
h2 = get_enthalpy(T2+273, P2*1e5, fluid)
T2_sat = get_saturation_temperature(P2*1e5, 1, fluid) - 273
print(f'P2 = {P2:.2f} barA, T2_sat = {T2_sat:.1f} C, T2 = {T2:.1f} C, h2 = {h2:.1f} kJ/kg')

P3 = P2                         # 응축기 출구 압력[barA]
T3_sat = get_saturation_temperature(P3*1e5, 0, fluid) - 273
T3 = T3_sat - SC_cond           # 응축기 출구 온도[C] (과냉각도 적용 53.0℃ 수준) 
h3 = get_enthalpy(T3+273, P3*1e5, fluid)    
print(f'P3 = {P3:.2f} barA, T3_sat = {T3_sat:.1f} C, T3 = {T3:.1f} C, h3 = {h3:.1f} kJ/kg')

P4 = P1                         # 증발기 입구 압력[PaA]
h4 = h3                         # 팽창 밸브 (등엔탈피 팽창)
print(f'P4 = {P4:.2f} barA, h4 = {h3:.1f} kJ/kg')
T4 = get_temperature(P4*1e5, h4*1000, fluid) - 273  # 증발기 입구 온도[K] (등엔탈피 팽창)
print(f'P4 = {P4:.2f} barA, T4 = {T4:.1f} C, h4 = {h4:.1f} kJ/kg')
# T4 = 9.0                        # Input 증발기 입구 온도[℃] 대안=13.0 [C]
x_evap = 0.34                   # Input 증발기 입구 건도
h4b = get_enthalpy_liq_gas(T4 + 273, P4 * 1e5, x_evap, fluid)
print(f'P4 = {P4:.2f} barA, T4 = {T4:.1f} C, h4b = {h4b:.1f} kJ/kg')
...
Phase1 = get_phase(T1 + 273, P1 * 1e5, fluid)
Phase2 = get_phase(T2 + 273, P2 * 1e5, fluid)   
Phase3 = get_phase(T3 + 273, P3 * 1e5, fluid)
Phase4 = get_phase(T4 + 273, P4 * 1e5, fluid)   


s1 = get_entropy(T1 + 273, P1 * 1e5, 1, fluid)
s2 = get_entropy(T2 + 273, P2 * 1e5, 1, fluid)
v1 = get_specific_volume(T1 + 273, P1 * 1e5, fluid)
v2 = get_specific_volume(T2 + 273, P2 * 1e5, fluid)
...
print(f"Point 4b: Phase={Phase4}, h={h4b:.1f} kJ/kg")
...
# 3. 압축기 요구성능 결정
# 압축기 출구 냉매 포화온도 계산
T2s = get_saturation_temperature(P2 * 1e5, 1, "R134a") - 273.15
...
sp_volume_cond = get_specific_volume(t_air_cond_in+273, Pair_in, "air")
...
T_air_sat = get_saturation_temperature(P2*1e5,1,"R134a") - 273
